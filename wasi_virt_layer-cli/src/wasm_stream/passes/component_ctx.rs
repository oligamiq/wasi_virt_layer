...
pub fn append_component_ctx(wasm_bytes: &[u8], ctx: &GeneratorCtx) -> eyre::Result<Vec<u8>> {
    ...
    let target_names = ctx.target_names.iter().map(|s| s.to_string()).collect::<Vec<_>>();

    eprintln!("[DEBUG COMPONENT_CTX] Appending context. threads={:?}, adjust_abi={}", ctx.threads, ctx.adjust_abi);
    add_custom(&mut out, "vfs_name", &serde_json::to_vec(&vfs_name)?);
    add_custom(&mut out, "target_names", &serde_json::to_vec(&target_names)?);
    add_custom(&mut out, "target_memory_type", &serde_json::to_vec(&ctx.target_memory_type)?);
    add_custom(&mut out, "unstable_print_debug", &serde_json::to_vec(&ctx.unstable_print_debug)?);
    add_custom(&mut out, "dwarf", &serde_json::to_vec(&ctx.dwarf)?);
    add_custom(&mut out, "threads", &serde_json::to_vec(&ctx.threads)?);
    add_custom(&mut out, "adjust_abi", &serde_json::to_vec(&ctx.adjust_abi)?);
        ...
        if let Payload::CustomSection(s) = payload? {
            ...
            match s.name() {
                ...
                "target_names" => {
                    ...
                    target_names = Some(cn.into_iter().map(CompactString::from).collect::<Box<_>>());
                }
                "target_memory_type" => target_memory_type = Some(serde_json::from_slice::<TargetMemoryType>(data)?),
                "unstable_print_debug" => unstable_print_debug = Some(serde_json::from_slice::<bool>(data)?),
                ...
                _ => {
                    eprintln!("[DEBUG COMPONENT_CTX] unhandled custom section: {}", s.name());
                }
            }
        }
    }
