...
pub fn translate_table_type<R: Rebind>(
...
pub fn translate_global_type<R: Rebind>(
    t: wasmparser::GlobalType,
    rebinder: &R,
...
pub fn translate_ref_type<R: Rebind>(
    r: wasmparser::RefType,
    rebinder: &R,
...
pub fn translate_heap_type<R: Rebind>(
    h: wasmparser::HeapType,
    rebinder: &R,
    ...
    match h {
        ...
        wasmparser::HeapType::Concrete(_) => {
            unimplemented!("Concrete heap type other than Module not supported yet")
        }
        ...
        wasmparser::HeapType::Exact(_) => {
            unimplemented!("Exact heap type other than Module not supported yet")
        }
    }
}

pub fn translate_sub_type<R: Rebind>(
    ...
    let composite_type = match &sub_ty.composite_type.inner {
        wasmparser::CompositeInnerType::Func(f) => {
            let params = f
                ...
                .iter()
                .copied()
                .map(|t| translate_val_type(t, rebinder))
                .collect::<Vec<_>>();
            let results = f
                .results()
                .iter()
            ...
            wasm_encoder::CompositeType {
                ...
                descriptor: sub_ty
                    .composite_type
                    .descriptor_idx
                    .map(|idx| rebinder.ty(idx.as_module_index().unwrap())),
                describes: sub_ty
                    .composite_type
                    .describes_idx
                    .map(|idx| rebinder.ty(idx.as_module_index().unwrap())),
...
pub fn translate_abstract_heap_type(
    t: wasmparser::AbstractHeapType,
...
pub fn translate<'a, R: Rebind>(op: &wasmparser::Operator<'a>, rebinder: &R) -> Instruction<'a> {
    match op {
        ...
        wasmparser::Operator::Block { blockty } => {
            Instruction::Block(translate_block_type(*blockty, rebinder))
        }
        wasmparser::Operator::Loop { blockty } => {
            Instruction::Loop(translate_block_type(*blockty, rebinder))
        }
        wasmparser::Operator::If { blockty } => {
            Instruction::If(translate_block_type(*blockty, rebinder))
        }
        ...
        wasmparser::Operator::Call { function_index } => {
            Instruction::Call(rebinder.function(*function_index))
        }
        wasmparser::Operator::CallIndirect {
            ...
            type_index: rebinder.ty(*type_index),
            table_index: rebinder.table(*table_index),
        },
        ...
        wasmparser::Operator::GlobalGet { global_index } => {
            Instruction::GlobalGet(rebinder.global(*global_index))
        }
        wasmparser::Operator::GlobalSet { global_index } => {
            Instruction::GlobalSet(rebinder.global(*global_index))
        }
        wasmparser::Operator::I32Load { memarg } => {
            Instruction::I32Load(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64Load { memarg } => {
            Instruction::I64Load(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::F32Load { memarg } => {
            Instruction::F32Load(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::F64Load { memarg } => {
            Instruction::F64Load(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32Load8S { memarg } => {
            Instruction::I32Load8S(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32Load8U { memarg } => {
            Instruction::I32Load8U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32Load16S { memarg } => {
            Instruction::I32Load16S(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32Load16U { memarg } => {
            Instruction::I32Load16U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64Load8S { memarg } => {
            Instruction::I64Load8S(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64Load8U { memarg } => {
            Instruction::I64Load8U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64Load16S { memarg } => {
            Instruction::I64Load16S(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64Load16U { memarg } => {
            Instruction::I64Load16U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64Load32S { memarg } => {
            Instruction::I64Load32S(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64Load32U { memarg } => {
            Instruction::I64Load32U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32Store { memarg } => {
            Instruction::I32Store(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64Store { memarg } => {
            Instruction::I64Store(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::F32Store { memarg } => {
            Instruction::F32Store(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::F64Store { memarg } => {
            Instruction::F64Store(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32Store8 { memarg } => {
            Instruction::I32Store8(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32Store16 { memarg } => {
            Instruction::I32Store16(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64Store8 { memarg } => {
            Instruction::I64Store8(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64Store16 { memarg } => {
            Instruction::I64Store16(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64Store32 { memarg } => {
            Instruction::I64Store32(translate_memarg(memarg, rebinder))
        }
        ...
        wasmparser::Operator::F32Const { value } => {
            Instruction::F32Const(f32::from_bits(value.bits()).into())
        }
        wasmparser::Operator::F64Const { value } => {
            Instruction::F64Const(f64::from_bits(value.bits()).into())
        }
        ...
        wasmparser::Operator::StructGet {
            ...
            struct_type_index: rebinder.ty(*struct_type_index),
        ...
        wasmparser::Operator::ArrayNew { array_type_index } => {
        ...
        wasmparser::Operator::ArrayNewFixed {
            ...
            array_type_index: rebinder.ty(*array_type_index),
        ...
        wasmparser::Operator::ArrayNewData {
            ...
            array_type_index: rebinder.ty(*array_type_index),
            array_data_index: rebinder.data(*array_data_index),
        },
        wasmparser::Operator::ArrayNewElem {
            ...
            array_type_index: rebinder.ty(*array_type_index),
            array_elem_index: rebinder.elem(*array_elem_index),
        },
        wasmparser::Operator::ArrayGet { array_type_index } => {
            Instruction::ArrayGet(*array_type_index)
        }
        wasmparser::Operator::ArrayGetS { array_type_index } => {
        ...
        wasmparser::Operator::ArraySet { array_type_index } => {
            Instruction::ArraySet(*array_type_index)
        }
        wasmparser::Operator::ArrayLen => Instruction::ArrayLen,
        wasmparser::Operator::ArrayFill { array_type_index } => {
            Instruction::ArrayFill(rebinder.ty(*array_type_index))
        }
        wasmparser::Operator::ArrayCopy {
            array_type_index_dst,
            array_type_index_src,
        } => Instruction::ArrayCopy {
            array_type_index_dst: rebinder.ty(*array_type_index_dst),
            array_type_index_src: rebinder.ty(*array_type_index_src),
        },
        ...
        wasmparser::Operator::RefTestNonNull { hty: _ } => {
            unimplemented!("RefTestNonNull: GC types not supported")
        }
        wasmparser::Operator::RefTestNullable { hty: _ } => {
            unimplemented!("RefTestNullable: GC types not supported")
        }
        wasmparser::Operator::RefCastNonNull { hty: _ } => {
            unimplemented!("RefCastNonNull: GC types not supported")
        }
        wasmparser::Operator::RefCastNullable { hty: _ } => {
            unimplemented!("RefCastNullable: GC types not supported")
        }
        wasmparser::Operator::BrOnCast {
            relative_depth: _,
            from_ref_type: _,
            to_ref_type: _,
        } => unimplemented!("Wasm GC cast not supported"),
        ...
        wasmparser::Operator::StructNewDesc { struct_type_index } => {
            Instruction::StructNewDesc(*struct_type_index)
        }
        wasmparser::Operator::StructNewDefaultDesc { struct_type_index } => {
            Instruction::StructNewDefaultDesc(*struct_type_index)
        }
        wasmparser::Operator::RefGetDesc { type_index } => {
            Instruction::RefGetDesc(rebinder.ty(*type_index))
        }
        wasmparser::Operator::RefCastDescEqNonNull { hty: _ } => {
            unimplemented!("RefCastDescEqNonNull: GC types not supported")
        }
        wasmparser::Operator::RefCastDescEqNullable { hty: _ } => {
            unimplemented!("RefCastDescEqNullable: GC types not supported")
        }
        wasmparser::Operator::BrOnCastDescEq {
            relative_depth: _,
            from_ref_type: _,
            to_ref_type: _,
        } => unimplemented!("BrOnCastDescEq: GC types not supported"),
        wasmparser::Operator::BrOnCastDescEqFail {
            relative_depth: _,
            from_ref_type: _,
            to_ref_type: _,
        } => unimplemented!("BrOnCastDescEqFail: GC types not supported"),
        ...
        wasmparser::Operator::MemoryInit { data_index, mem } => Instruction::MemoryInit {
            mem: rebinder.memory(*mem),
            data_index: rebinder.data(*data_index),
        },
        wasmparser::Operator::DataDrop { data_index } => {
            Instruction::DataDrop(rebinder.data(*data_index))
        }
        wasmparser::Operator::MemoryCopy { dst_mem, src_mem } => Instruction::MemoryCopy {
            src_mem: rebinder.memory(*src_mem),
            dst_mem: rebinder.memory(*dst_mem),
        },
        wasmparser::Operator::MemoryFill { mem } => Instruction::MemoryFill(rebinder.memory(*mem)),
        wasmparser::Operator::TableInit { elem_index, table } => Instruction::TableInit {
            table: rebinder.table(*table),
            elem_index: rebinder.elem(*elem_index),
        },
        wasmparser::Operator::ElemDrop { elem_index } => {
            Instruction::ElemDrop(rebinder.elem(*elem_index))
        }
        wasmparser::Operator::TableCopy {
            dst_table,
            src_table,
        } => Instruction::TableCopy {
            src_table: rebinder.table(*src_table),
            dst_table: rebinder.table(*dst_table),
        },
        wasmparser::Operator::TypedSelect { ty } => {
            Instruction::TypedSelect(translate_val_type(*ty, rebinder))
        }
        wasmparser::Operator::TypedSelectMulti { tys: _ } => {
            unimplemented!("TypedSelectMulti: GC types not supported")
        }
        wasmparser::Operator::RefNull { hty: _ } => unimplemented!("RefNull not supported"),
        wasmparser::Operator::RefIsNull => Instruction::RefIsNull,
        wasmparser::Operator::RefFunc { function_index } => {
            Instruction::RefFunc(rebinder.function(*function_index))
        }
        ...
        wasmparser::Operator::ReturnCall { function_index } => {
            Instruction::ReturnCall(rebinder.function(*function_index))
        }
        ...
        wasmparser::Operator::MemoryDiscard { mem } => {
            Instruction::MemoryDiscard(rebinder.memory(*mem))
        }
        wasmparser::Operator::MemoryAtomicNotify { memarg } => {
            Instruction::MemoryAtomicNotify(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::MemoryAtomicWait32 { memarg } => {
            Instruction::MemoryAtomicWait32(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::MemoryAtomicWait64 { memarg } => {
            Instruction::MemoryAtomicWait64(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::AtomicFence => Instruction::AtomicFence,
        wasmparser::Operator::I32AtomicLoad { memarg } => {
            Instruction::I32AtomicLoad(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicLoad { memarg } => {
            Instruction::I64AtomicLoad(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicLoad8U { memarg } => {
            Instruction::I32AtomicLoad8U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicLoad16U { memarg } => {
            Instruction::I32AtomicLoad16U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicLoad8U { memarg } => {
            Instruction::I64AtomicLoad8U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicLoad16U { memarg } => {
            Instruction::I64AtomicLoad16U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicLoad32U { memarg } => {
            Instruction::I64AtomicLoad32U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicStore { memarg } => {
            Instruction::I32AtomicStore(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicStore { memarg } => {
            Instruction::I64AtomicStore(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicStore8 { memarg } => {
            Instruction::I32AtomicStore8(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicStore16 { memarg } => {
            Instruction::I32AtomicStore16(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicStore8 { memarg } => {
            Instruction::I64AtomicStore8(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicStore16 { memarg } => {
            Instruction::I64AtomicStore16(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicStore32 { memarg } => {
            Instruction::I64AtomicStore32(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmwAdd { memarg } => {
            Instruction::I32AtomicRmwAdd(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmwAdd { memarg } => {
            Instruction::I64AtomicRmwAdd(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw8AddU { memarg } => {
            Instruction::I32AtomicRmw8AddU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw16AddU { memarg } => {
            Instruction::I32AtomicRmw16AddU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw8AddU { memarg } => {
            Instruction::I64AtomicRmw8AddU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw16AddU { memarg } => {
            Instruction::I64AtomicRmw16AddU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw32AddU { memarg } => {
            Instruction::I64AtomicRmw32AddU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmwSub { memarg } => {
            Instruction::I32AtomicRmwSub(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmwSub { memarg } => {
            Instruction::I64AtomicRmwSub(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw8SubU { memarg } => {
            Instruction::I32AtomicRmw8SubU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw16SubU { memarg } => {
            Instruction::I32AtomicRmw16SubU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw8SubU { memarg } => {
            Instruction::I64AtomicRmw8SubU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw16SubU { memarg } => {
            Instruction::I64AtomicRmw16SubU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw32SubU { memarg } => {
            Instruction::I64AtomicRmw32SubU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmwAnd { memarg } => {
            Instruction::I32AtomicRmwAnd(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmwAnd { memarg } => {
            Instruction::I64AtomicRmwAnd(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw8AndU { memarg } => {
            Instruction::I32AtomicRmw8AndU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw16AndU { memarg } => {
            Instruction::I32AtomicRmw16AndU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw8AndU { memarg } => {
            Instruction::I64AtomicRmw8AndU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw16AndU { memarg } => {
            Instruction::I64AtomicRmw16AndU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw32AndU { memarg } => {
            Instruction::I64AtomicRmw32AndU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmwOr { memarg } => {
            Instruction::I32AtomicRmwOr(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmwOr { memarg } => {
            Instruction::I64AtomicRmwOr(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw8OrU { memarg } => {
            Instruction::I32AtomicRmw8OrU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw16OrU { memarg } => {
            Instruction::I32AtomicRmw16OrU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw8OrU { memarg } => {
            Instruction::I64AtomicRmw8OrU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw16OrU { memarg } => {
            Instruction::I64AtomicRmw16OrU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw32OrU { memarg } => {
            Instruction::I64AtomicRmw32OrU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmwXor { memarg } => {
            Instruction::I32AtomicRmwXor(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmwXor { memarg } => {
            Instruction::I64AtomicRmwXor(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw8XorU { memarg } => {
            Instruction::I32AtomicRmw8XorU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw16XorU { memarg } => {
            Instruction::I32AtomicRmw16XorU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw8XorU { memarg } => {
            Instruction::I64AtomicRmw8XorU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw16XorU { memarg } => {
            Instruction::I64AtomicRmw16XorU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw32XorU { memarg } => {
            Instruction::I64AtomicRmw32XorU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmwXchg { memarg } => {
            Instruction::I32AtomicRmwXchg(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmwXchg { memarg } => {
            Instruction::I64AtomicRmwXchg(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw8XchgU { memarg } => {
            Instruction::I32AtomicRmw8XchgU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw16XchgU { memarg } => {
            Instruction::I32AtomicRmw16XchgU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw8XchgU { memarg } => {
            Instruction::I64AtomicRmw8XchgU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw16XchgU { memarg } => {
            Instruction::I64AtomicRmw16XchgU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw32XchgU { memarg } => {
            Instruction::I64AtomicRmw32XchgU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmwCmpxchg { memarg } => {
            Instruction::I32AtomicRmwCmpxchg(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmwCmpxchg { memarg } => {
            Instruction::I64AtomicRmwCmpxchg(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw8CmpxchgU { memarg } => {
            Instruction::I32AtomicRmw8CmpxchgU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I32AtomicRmw16CmpxchgU { memarg } => {
            Instruction::I32AtomicRmw16CmpxchgU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw8CmpxchgU { memarg } => {
            Instruction::I64AtomicRmw8CmpxchgU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw16CmpxchgU { memarg } => {
            Instruction::I64AtomicRmw16CmpxchgU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::I64AtomicRmw32CmpxchgU { memarg } => {
            Instruction::I64AtomicRmw32CmpxchgU(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load { memarg } => {
            Instruction::V128Load(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load8x8S { memarg } => {
            Instruction::V128Load8x8S(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load8x8U { memarg } => {
            Instruction::V128Load8x8U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load16x4S { memarg } => {
            Instruction::V128Load16x4S(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load16x4U { memarg } => {
            Instruction::V128Load16x4U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load32x2S { memarg } => {
            Instruction::V128Load32x2S(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load32x2U { memarg } => {
            Instruction::V128Load32x2U(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load8Splat { memarg } => {
            Instruction::V128Load8Splat(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load16Splat { memarg } => {
            Instruction::V128Load16Splat(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load32Splat { memarg } => {
            Instruction::V128Load32Splat(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load64Splat { memarg } => {
            Instruction::V128Load64Splat(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load32Zero { memarg } => {
            Instruction::V128Load32Zero(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load64Zero { memarg } => {
            Instruction::V128Load64Zero(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Store { memarg } => {
            Instruction::V128Store(translate_memarg(memarg, rebinder))
        }
        wasmparser::Operator::V128Load8Lane { memarg, lane } => Instruction::V128Load8Lane {
            memarg: translate_memarg(memarg, rebinder),
            lane: *lane,
        },
        wasmparser::Operator::V128Load16Lane { memarg, lane } => Instruction::V128Load16Lane {
            memarg: translate_memarg(memarg, rebinder),
            lane: *lane,
        },
        wasmparser::Operator::V128Load32Lane { memarg, lane } => Instruction::V128Load32Lane {
            memarg: translate_memarg(memarg, rebinder),
            lane: *lane,
        },
        wasmparser::Operator::V128Load64Lane { memarg, lane } => Instruction::V128Load64Lane {
            memarg: translate_memarg(memarg, rebinder),
            lane: *lane,
        },
        wasmparser::Operator::V128Store8Lane { memarg, lane } => Instruction::V128Store8Lane {
            memarg: translate_memarg(memarg, rebinder),
            lane: *lane,
        },
        wasmparser::Operator::V128Store16Lane { memarg, lane } => Instruction::V128Store16Lane {
            memarg: translate_memarg(memarg, rebinder),
            lane: *lane,
        },
        wasmparser::Operator::V128Store32Lane { memarg, lane } => Instruction::V128Store32Lane {
            memarg: translate_memarg(memarg, rebinder),
            lane: *lane,
        },
        wasmparser::Operator::V128Store64Lane { memarg, lane } => Instruction::V128Store64Lane {
            memarg: translate_memarg(memarg, rebinder),
            lane: *lane,
        },
        ...
        wasmparser::Operator::TryTable { try_table } => Instruction::TryTable(
            translate_block_type(try_table.ty, rebinder),
            std::borrow::Cow::Owned(
                try_table
                    .catches
                    .iter()
                    .map(|c| translate_catch(c, rebinder))
                    .collect(),
            ),
        ),
        ...
        wasmparser::Operator::Try { blockty } => {
            Instruction::Try(translate_block_type(*blockty, rebinder))
        }
        ...
        wasmparser::Operator::ContNew { cont_type_index: _ } => unimplemented!("ContNew"),
        wasmparser::Operator::ContBind {
            argument_index: _,
            result_index: _,
        } => unimplemented!("ContBind"),
        wasmparser::Operator::Suspend { tag_index: _ } => unimplemented!("Suspend"),
        ...
        wasmparser::Operator::ResumeThrow {
            ...
            tag_index: _,
            resume_table: _,
        } => unimplemented!("ResumeThrow"),
        wasmparser::Operator::ResumeThrowRef {
            cont_type_index: _,
            resume_table: _,
        } => unimplemented!("ResumeThrowRef"),
        wasmparser::Operator::Switch {
            cont_type_index: _,
            tag_index: _,
...
pub fn translate_val_type<R: Rebind>(
...
pub fn translate_block_type<R: Rebind>(
    bty: wasmparser::BlockType,
    rebinder: &R,
) -> wasm_encoder::BlockType {
    match bty {
        wasmparser::BlockType::Empty => wasm_encoder::BlockType::Empty,
        wasmparser::BlockType::Type(ty) => {
            wasm_encoder::BlockType::Result(translate_val_type(ty, rebinder))
        }
        wasmparser::BlockType::FuncType(idx) => {
            wasm_encoder::BlockType::FunctionType(rebinder.ty(idx))
        }
    }
}
...
pub fn translate_memarg<R: Rebind>(
...
pub fn translate_const_expr<R: Rebind>(
    ...
    match op {
        ...
        wasmparser::Operator::F32Const { value } => Ok(wasm_encoder::ConstExpr::f32_const(
            f32::from_bits(value.bits()).into(),
        )),
        wasmparser::Operator::F64Const { value } => Ok(wasm_encoder::ConstExpr::f64_const(
            f64::from_bits(value.bits()).into(),
        )),
        wasmparser::Operator::GlobalGet { global_index } => Ok(
            wasm_encoder::ConstExpr::global_get(rebinder.global(global_index)),
        ),
