...
impl Generator for PollWait {
    fn post_combine(
        ...
        if let Some(id) = ("wvl_poll", "__wvl_poll_atomic_wait")
            ...
            .ok()
        {
            if ctx.threads {
                ...
                module
                    ...
                    .map_err(|e| {
                        eyre::eyre!(
                            "Failed to replace __wvl_poll_atomic_wait with atomic.wait: {e}"
                        )
                    })?;
            } else {
                module
                    .replace_imported_func(id, |(builder, _args)| {
                        builder.func_body().i32_const(2); // 2 means timed out
                    })
                    .map_err(|e| {
                        eyre::eyre!(
                            "Failed to replace __wvl_poll_atomic_wait with dummy return: {e}"
                        )
                    })?;
            }
        }
