...
import_wasm!(pool_reused_start_target);
...
impl Guest for ComponentABI {
    fn main() {
        ...
        println!("Starting pool reused start-section test");
        pool_reused_start_target::_start();
        println!("Pool reused start-section test passed");
    }
}
...
plug_thread!({ &THREAD_POOL }, pool_reused_start_target, self);
plug_poll!(DefaultWaitPoll, pool_reused_start_target);

wasi_virt_layer::plug_clock!(
    wasi_virt_layer::clock::StandardClock,
    pool_reused_start_target,
    self
);
...
mod env {
    ...
    plug_env!(@embedded, HostEnvTy, pool_reused_start_target, self);
}
...
mod fs {
    ...
    plug_fs!(&VIRTUAL_FILE_SYSTEM, pool_reused_start_target, self);
}
