...
use crate::{
    ...
    wasi::file::{
        FilestatWithoutDevice, Wasip1FileSystem, Wasip1FileTrait, Wasip1LFS, stdio::StdIO,
    },
};
...
macro_rules! ConstFiles {
    (
        ...
        $crate::wasi::file::constant::VFSConstNormalFiles::new({
            const COUNT: usize = {
                let mut count = 0;
    ...
    (@next, $depth:expr, $static_array:ident, [$empty:expr], [$parent_path:expr], [$name:expr], [
        ...
        $static_array.push(($depth, (
            $parent_path,
            $name,
            $crate::wasi::file::constant::VFSConstNormalInode::Dir(
                get_child_range(
                    $empty,
                    $parent_path,
                    &$static_array
    ...
    (@next, $depth:expr, $static_array:ident, [$empty:expr], [$path:expr], [$name:expr], $file:tt) => {
        $static_array.push((
            ...
            (
                $path,
                $name,
            $crate::wasi::file::constant::VFSConstNormalInode::File($file, get_parent($empty, $path, &$static_array).unwrap())
        )));
    };
}
